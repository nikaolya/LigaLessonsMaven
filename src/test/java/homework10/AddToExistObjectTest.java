...
public class AddToExistObjectTest {
    ...
    public static Collection<Object[]> setData() {
        return Arrays.asList(new Object[][]{
                {"elephant", 3, 8, true},
                {"elephant", -1, 5, false}, // этот тест падает,
                // так как при отрицательном числе вместо сложения получается вычитание и как следствие подсчет элементов неверен,
                // плюс не понятно как вообще операция добавления отрицательного числа элементов может считаться успешной
                {"elephant", 0, 5, false}, // этот тест тоже падает,
